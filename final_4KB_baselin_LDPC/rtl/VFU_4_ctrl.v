module VFU_4_ctrl (
        ...
        output reg  [63:0] bit_data_reg,   // 硬判决数据（96 位宽）
    ...
    reg  [63:0] bit_data;                 // 硬判决数据寄存器（96 位宽）
    ...
    always @(posedge sys_clk or negedge sys_rst_n)
        if (!sys_rst_n)
            bit_data <= 64'd0;  // 复位时清零
        else if (flag_VFU_end == 1'b1 || flag_VFU_start == 1'b1)
            bit_data <= 64'd0;  // 结束信号或开始信号时清零
        else if (cnt_rd_wr == 1'b1)
            bit_data <= {cn_all_sum[2], bit_data[63:1]};  // 将当前列的最高位插入到硬判决数据中
        else
            bit_data <= bit_data;

    always @(posedge sys_clk or negedge sys_rst_n)
        if (!sys_rst_n)
            bit_data_reg <= 64'd0;  // 复位时清零
        else if (flag_VFU_start == 1'b1)
            bit_data_reg <= 64'd0;  // 开始信号时清零
